...
document.getElementById('oldPrice').innerText = oldPrice ? `$${oldPrice}` : '';
...
fetch(url) // استبدل برابط API الحقيقي
    ...
    .then(data => {
        ...
        const relatedProducts = data.items.filter(product =>
            product.fields?.category === currentCategory && product.fields?.name !== currentProductName
        );

        const relatedContainer = document.querySelector('.row.justify-content-center');
        ...
        relatedProducts.forEach(product => {
            ...
            const imageUrl = assetMap[fields.image?.sys?.id] || 'https://dummyimage.com/450x300/dee2e6/6c757d.jpg';
            const oldPrice = fields.oldPrice ? `$${fields.oldPrice}` : '';

            const productCard = `
                <div class="col mb-5">
                    <div class="card h-100">
                        <img class="card-img-top" src="${imageUrl}" alt="${fields.name || 'No Name'}" />
                        <div class="card-body p-4">
                            <div class="text-center">
                                <h5 class="fw-bolder">${fields.name || 'No Name'}</h5>
                                <div class="fs-5">
                        ...
                        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                            <div class="text-center">
                                <a class="btn btn-outline-dark mt-auto" href="product-detils.html?name=${encodeURIComponent(fields.name || '')}&image=${encodeURIComponent(imageUrl)}&price=${fields.price || ''}&oldPrice=${fields.oldPrice || ''}&category=${fields.category || ''}&description=${encodeURIComponent(fields.description || '')}">
                                    View Details
                                </a>
                            </div>
                        </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (addToCartBtn) {
            addToCartBtn.addEventListener("click", function () {
                let productImage = document.getElementById("productImage")?.src || "https://via.placeholder.com/150";
                let product = {
                    name: document.getElementById("productName").innerText || "Unknown Product",
                    price: document.getElementById("productPrice").innerText.replace("$", "") || "0",
                    oldPrice: document.getElementById("oldPrice").innerText.replace("$", "") || "0",
                    category: document.getElementById("productCategory").innerText || "Uncategorized",
                    image: productImage,
